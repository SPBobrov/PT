...
const Timer = {
  ...
  tick() {
    ...
    if (remaining <= 0) {
      ...
      if (this.currentMode === 'work') this.saveSession(this.totalDuration);
  ...
  saveSession(actualDuration) {
    ...
    fetch('/api/sessions', {
      ...
      body: JSON.stringify({ activityType, duration: actualDuration, comment }),
    })
      .then(response => {
  ...
  stop() {
    ...
    if (this.currentMode === 'work' && elapsed > 0) {
